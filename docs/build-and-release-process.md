...
2. (Optional) If `assets/png/*.png` exists, build tooling may generate auxiliary assets (currently unused by the e‑ink UI).
...
This project targets a single e‑ink display backend (IT8951). The `DISPLAY_DRIVER` selector remains fixed to `DISPLAY_DRIVER_IT8951` and the implementation is included via the sketch-root translation unit `src/app/display_drivers.cpp`.
